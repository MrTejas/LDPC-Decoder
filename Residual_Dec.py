...
class Residual_Dec:
    ...
    def pseudo_row_update(self, a):
        # creating pseudo variables to simulate the scheduling of cluster 'a'
        p_sum = self.sum
        p_L = self.L
    ...
    # probably not needed
    def pseudo_column_update(self, a, p_sum, p_L):
        for j in range(self.num_VN):
            tot = 0
            for i in self.clusters[a]:
                if not np.isnan(p_L[i,j]):
    ...
    def get_cluster_residual(self, a):
        p_L, p_sum = self.pseudo_row_update(a)
        sum_change = 0
        it = 0
        for i in self.clusters[a]:
            for j in self.VN[i]:
                sum_change = sum_change + (np.abs(p_L[i,j]-self.L[i,j]))
                it = it + 1
        return sum_change/(it)
    ...
    def get_next_cluster(self,iter_number):
        ...
        for a in range(self.num_clusters):
            residuals.append(self.get_cluster_residual(a))
        a_star = np.argmax(np.array(residuals))
        return a_star
